...
2. `CFPreferences` plumbing → **`NSUserDefaults` suite `com.nicksworks.dottoplusplus.prefs`**
   (the current rebuild's domain/plist via cfprefsd; works identically in SpringBoard and Settings).
...
5. `_frameForAccessoryView:` hook → removed; `accessoryCenterForIconBounds:` hook
   added (returns the width-aware `(iconWidth - 2, 2)` anchor, with `(58, 2)` fallback).
6. Stock-restore path → an associated snapshot restores all dotto-owned view state (iOS 17-safe); this is broader than the original captured-image-only pass.
...
8. Package schemes: **roothide** and **rootless** (`THEOS_PACKAGE_SCHEME`),
   with arm64/arm64e matrix builds and `Depends: firmware (>= 15.0), firmware (<< 18.0), ellekit, preferenceloader`.

## 5. Geometry

iPhone icon grid (measured on-device, iOS 17.1.1): home icons 60 pt (pitch 90),
dock 64 pt. The badge geometry is a 26×26 badge with a 2pt trailing inset:
center `(iconWidth - 2, 2)` gives `(58,2)` on a 60 pt icon and `(62,2)` on a
64 pt dock icon. This reproduces the dotto look: a dot hanging off the icon's
top-right corner (`SBIconView` does not clip subviews).

## 6. Current stabilization pass

The historical sections above describe the original binary and the first iOS 17 rebuild. The current source is not identical to that historical implementation:

- `Tweak.x` uses a weak badge-view registry, honors the object returned by `%orig` from `-init`, confines notification/reapply work to the main queue, and guards private KVC lookups.
- Badge state is captured before dotto++ mutates it and restored symmetrically when disabled or reconfigured. Folder colors bypass the app cache because their input is live badge membership; app entries are invalidated on reconfiguration.
- `UIColor+dottoPlusPlus` uses floating-point aspect-preserving downsampling, starts cluster dominance at one, returns a real single-cluster color, and returns `nil` for extraction failure instead of using red as an error sentinel. Pastel mode defaults to dotto+ 1.0.6's original HSV brightness behavior via `dpp_faithfulPastelColor`; the optional perceptual mode uses `dpp_oklabPastelColor`.
- Settings uses one canonical palette and persisted preferences as its selected-color authority. Native picker changes are committed once on finish rather than notifying SpringBoard for every drag sample.
- `accessoryCenterForIconBounds:` is installed only in the iOS 17 Logos group; the common `_centerForAccessoryView` hook remains separate.

These changes are source-level claims until the build and the connected-device scenarios in `AUDIT.md` are recorded as verified.
